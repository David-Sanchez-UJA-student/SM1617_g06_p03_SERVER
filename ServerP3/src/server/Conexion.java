	...
	public void run() {
		...
		if (mSocket != null) {
			try {
				...
				// Conexion con la entrada y salida del socket
				...
				// Se envia el mensaje de bienvenida
				...
				while ((inputData = inputStream.readLine()) != null && !salir) {
					...
					if (fields.length == 1) {// No hay espacios, debe ser solo
						...
						parametro = null;
													...
													// ya qe el metodo split ha
													// devuelto dos o mas
													// cadenas
						comando = fields[0];// El comando debe ser el primer
											// campo
						// Se extrae el parametro que se considera que es todo
					...
					case S_USER:// Estado USER
						if (comando.equalsIgnoreCase(QUIT)) {// En todos los
							...
							tempUser = parametro;
							outputData = OK + CRLF;
							estado++;// Se incrementa el estado aunque aun no se
										// compara
						} else {// No es un comando valido
